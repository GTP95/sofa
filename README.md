...
Stable example profiles are available in [`profiles/examples/`](profiles/examples/), including AES, KECCAK, the three ASCON targets, and the Raspberry Pi challenge. The root-level profiles generated by `make` are ignored because they are overwritten by builds and removed by `make clean`; use an example profile unless you intentionally need one generated for custom build options.
